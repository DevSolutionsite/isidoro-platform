...
**Última actualización:** 10 de agosto de 2026 — Fran (`/admin/usuarios` — admin puede cambiar rol de cualquier usuario, cierra punto 4 de DEC-036, ver DEC-041; migración de `categories.is_active` escrita para Kevin, punto 5 en curso, ver DEC-042). Kevin agregó sistema de mails promocionales con Resend (`/admin/email`) el mismo día.
...
| Envío de mails promocionales (Resend) — `/admin/email` | Kevin | ✅ Completado | Fuera del alcance de Fran (aclarado explícitamente en DEC-036: "el envío de mails con promociones lo maneja Kevin del lado de infraestructura"). Agregado por Kevin directo a `main` el 10 de agosto: `EmailForm.tsx` + `admin-email.ts`, plantilla `promotionalTemplate.ts`, opt-out (`profiles.email_opt_out`, `unsubscribe_token`) con página pública `/unsubscribe`, bucket de Storage para imágenes de email. Migraciones `20260810120000_profiles_email_opt_out.sql` y `20260810120100_email_images_storage.sql`. Fran no revisó el detalle de esta implementación — no es su dominio. |
